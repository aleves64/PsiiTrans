...
void TextThread::Push(BYTE* data, int length)
{
	...
	if (length == 1) // doublebyte characters must be processed as pairs
	{
		...
		else if (IsDBCSLeadByteEx(hp.codepage ? hp.codepage : defaultCodepage, data[0]))
		{
			leadByte = data[0];
			length = 0;
		}
	...
	else if (auto converted = StringToWideString(std::string((char*)data, length), hp.codepage ? hp.codepage : defaultCodepage)) buffer.append(converted.value());
	else AddConsoleOutput(INVALID_CODEPAGE);
...
void TextThread::Flush()
{
	...
	struct OutputInfo outinfo = { this->handle, this->tp.processId, this->tp.addr, this->tp.ctx, this->tp.ctx2, this->name.c_str() };

	for (auto& sentence : sentences)
	{
		totalSize += sentence.size();
		sentence.erase(std::remove(sentence.begin(), sentence.end(), 0), sentence.end());
		if (Output(outinfo, sentence.c_str())) storage->append(sentence);
	}
