...
namespace
{
	...
	public:
		...
		HookEventHandler OnHookFound = [](HookParam hp, std::wstring text)
		{
			AddConsoleOutput(HookCode::Generate(hp) + L": " + text);
		};
	...
	ProcessEventHandler OnConnect, OnDisconnect;
	ThreadEventHandler OnCreate, OnDestroy;
		...
		std::thread([]
		{
			...
			while (ReadFile(hookPipe, buffer, PIPE_BUFFER_SIZE, &bytesRead, nullptr))
				...
				case HOST_NOTIFICATION_TEXT:
				{
					...
					AddConsoleOutput(StringToWideString(info.message));
				}
				break;
				default:
				{
...
void __stdcall Start(ProcessEventHandler Connect, ProcessEventHandler Disconnect, ThreadEventCallback Create, ThreadEventCallback Destroy, TextThread::OutputCallback Output)
{
	OnConnect = Connect;
	OnDisconnect = Disconnect;
	OnCreate = [Create](TextThread& thread) {
		struct OutputInfo outinfo = {
			thread.handle,
			thread.tp.processId,
			thread.tp.addr,
			thread.tp.ctx,
	...
	OnDestroy = [Destroy](TextThread& thread) {
		struct OutputInfo outinfo = {
			thread.handle,
			thread.tp.processId,
			thread.tp.addr,
...
void __stdcall InjectProcess(DWORD processId)
{
...
void __stdcall DetachProcess(DWORD processId)
{
	processRecordsByIds->at(processId).Send(HOST_COMMAND_DETACH);
}

void __stdcall InsertHook(DWORD processId, HookParam hp)
{
	processRecordsByIds->at(processId).Send(InsertHookCmd(hp));
}

void __stdcall RemoveHook(DWORD processId, uint64_t address)
{
